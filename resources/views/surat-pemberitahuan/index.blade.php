                                ...
                                <tbody>
                                    @forelse($suratPemberitahuan as $index => $item)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($item->jadwalFasilitasi)
                                                    {{ $item->jadwalFasilitasi->tanggal_mulai->format('d M Y') }} -
                                                    {{ $item->jadwalFasilitasi->tanggal_selesai->format('d M Y') }}
                                                @else
                                                    -
                                                @endif
                                            </td>
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                            href="{{ route('surat-pemberitahuan.show', $item) }}">
                                                            <i class="bx bx-show me-1"></i> Detail
                                                        </a>
                                                        <a class="dropdown-item"
                                                            href="{{ route('surat-pemberitahuan.edit', $item) }}">
                                                            <i class="bx bx-edit-alt me-1"></i> Edit
                                                        </a>
                                                        @if ($item->file_path)
                                                            <a class="dropdown-item"
                                                                href="{{ route('surat-pemberitahuan.download', $item) }}"
                                                                target="_blank">
                                                                <i class="bx bx-download me-1"></i> Download
                                                            </a>
                                                        @endif
                                                        @if ($item->status == 'draft')
                                                            <form action="{{ route('surat-pemberitahuan.send', $item) }}"
                                                        ...
                                                        @endif
                                                        <form action="{{ route('surat-pemberitahuan.destroy', $item) }}"
                                                            method="POST" class="d-inline">
                                                            @csrf
                                                            @method('DELETE')
