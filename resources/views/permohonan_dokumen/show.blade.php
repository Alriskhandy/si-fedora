                ...
                <div class="card-body">
                    <table class="table table-borderless">
                        <tr>
                            ...
                            <td width="5%">:</td>
                            <td>{{ $permohonanDokumen->permohonan->nomor_permohonan ?? '-' }} - {{ $permohonanDokumen->permohonan->nama_dokumen ?? '-' }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($permohonanDokumen->status_verifikasi)
                                    <span class="badge bg-label-{{ $permohonanDokumen->status_verifikasi_badge_class }}">{{ $permohonanDokumen->status_verifikasi_label }}</span>
                                    @if($permohonanDokumen->catatan_verifikasi)
                                        <br><small class="text-muted">Catatan: {{ $permohonanDokumen->catatan_verifikasi }}</small>
                                    @endif
                                @else
                        ...
                        <tr>
                            ...
                            <td>:</td>
                            <td>{{ $permohonanDokumen->created_at ? $permohonanDokumen->created_at->format('d M Y H:i') : '-' }}</td>
                        </tr>
                        <tr>
                            <td><strong>Diupdate</strong></td>
                            <td>:</td>
                            <td>{{ $permohonanDokumen->updated_at ? $permohonanDokumen->updated_at->format('d M Y H:i') : '-' }}</td>
                        </tr>
                    </table>

                    <div class="mt-4">
                        @if($permohonanDokumen->status_verifikasi !== 'verified')
                        <a href="{{ route('permohonan-dokumen.edit', $permohonanDokumen) }}" class="btn btn-primary me-2">
